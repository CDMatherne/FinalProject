...
class AdvancedAnalysis:
    ...
    def __init__(self, parent_window, output_directory=None, config_path='config.ini'):
        ...
        # Set output_directory but use get_output_directory() for actual file saves
        self.output_directory = output_directory or self.run_info['output_directory']
        # Ensure the output directory exists
        self.get_output_directory()  # This will create it if needed
    ...
    def get_output_directory(self):
        """
        Get the unified output directory for all advanced analysis outputs.
        Uses either hardcoded path "C:\\AIS_Data\\Output" or output_directory from config.
        ...
        hardcoded_path = Path("C:\\AIS_Data\\Output")
        
        # Use hardcoded path if it exists or can be created
        if hardcoded_path.exists() or hardcoded_path.parent.exists():
            output_dir = hardcoded_path
        else:
    ...
    def get_map_output_directory(self):
        ...
        Uses the unified output directory from get_output_directory().
        
        Returns:
            Path object for the map output directory
        """
        map_dir = self.get_output_directory() / "Path_Maps"
        ...
        try:
            ...
            try:
                ...
                if os.path.exists(consolidated_path):
                    ...
                    if ship_types and len(ship_types) > 0:
                        # Use exact VesselType matching instead of MainVesselType to avoid range-based selection
                        # This ensures that selecting type 30 or 50 doesn't select all 30-series or 50-series vessels
                        if 'VesselType' in self._cached_data.columns:
                            vessel_types_int = [int(st) for st in ship_types]
                            self._cached_data['VesselType'] = pd.to_numeric(self._cached_data['VesselType'], errors='coerce').astype('Int64')
                            before_filter = len(self._cached_data)
                            self._cached_data = self._cached_data[self._cached_data['VesselType'].isin(vessel_types_int)].copy()
                            logger.info(f"Filtered consolidated data by exact VesselType: {before_filter} -> {len(self._cached_data)} records")
                        elif 'MainVesselType' in self._cached_data.columns:
                            # Fallback to MainVesselType only if VesselType column doesn't exist
                            before_filter = len(self._cached_data)
        ...
        try:
            ...
            if df.empty:
                ...
                if output_path is None:
                    output_path = os.path.join(str(self.get_output_directory()), f"Dataset_Info_{timestamp}.csv")
                ...
                info_df = pd.DataFrame([
                    ...
                    {"Parameter": "Output Directory", "Value": str(self.get_output_directory())},
            ...
            if output_path is None:
                timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
                output_path = os.path.join(str(self.get_output_directory()), f"Full_Dataset_{timestamp}.csv")
    ...
    def generate_summary_report(self, output_path=None):
        ...
        try:
            ...
            if df.empty:
                ...
                timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
                if output_path is None:
                    output_path = os.path.join(str(self.get_output_directory()), f"Summary_Report_{timestamp}.html")
                ...
                self._open_html_file(output_path)
                return output_path
            ...
            # Add section for vessels with multiple anomaly types at the bottom
            if not anomaly_df.empty and 'AnomalyType' in anomaly_df.columns and 'MMSI' in anomaly_df.columns:
                # Find vessels with multiple anomaly types
                vessels_with_multiple_types = []
                
                for mmsi in anomaly_df['MMSI'].unique():
                    vessel_anomalies = anomaly_df[anomaly_df['MMSI'] == mmsi]
                    unique_types = vessel_anomalies['AnomalyType'].nunique()
                    
                    if unique_types > 1:  # Multiple anomaly types
                        # Get vessel info from main dataframe
                        vessel_info = {}
                        if not df.empty and 'MMSI' in df.columns:
                            ...
                            if not vessel_data.empty:
                                if 'VesselName' in vessel_data.columns:
                                    ...
                                    vessel_info['name'] = names[0] if len(names) > 0 else 'Unknown'
                                else:
                                    vessel_info['name'] = 'Unknown'
                                
                                if 'VesselType' in vessel_data.columns:
                                    types = vessel_data['VesselType'].dropna().unique()
                                    vessel_type = int(types[0]) if len(types) > 0 and pd.notna(types[0]) else None
                                    if vessel_type is not None:
                                        type_name = get_vessel_type_name(vessel_type)
                                        # Extract main category (e.g., "Cargo" from "Cargo, all ships of this type")
                                        main_category = type_name.split(',')[0].strip()
                                        vessel_info['type'] = f"{main_category} [{vessel_type}]"
                                    else:
                                        vessel_info['type'] = 'Unknown'
                                else:
                                    vessel_info['type'] = 'Unknown'
                            else:
                                vessel_info['name'] = 'Unknown'
                            ...
                            # Try to get from anomaly data if available
                            ...
                            if 'VesselType' in vessel_anomalies.columns:
                                types = vessel_anomalies['VesselType'].dropna().unique()
                                vessel_type = int(types[0]) if len(types) > 0 and pd.notna(types[0]) else None
                                if vessel_type is not None:
                                    type_name = get_vessel_type_name(vessel_type)
                                    main_category = type_name.split(',')[0].strip()
                        ...
                        anomaly_counts = vessel_anomalies['AnomalyType'].value_counts().to_dict()
                        anomaly_breakdown = ', '.join([f"{atype}: {count}" for atype, count in sorted(anomaly_counts.items())])
                        
                        vessels_with_multiple_types.append({
                            'mmsi': mmsi,
                            'name': vessel_info.get('name', 'Unknown'),
                            'type': vessel_info.get('type', 'Unknown'),
                            'anomaly_breakdown': anomaly_breakdown,
                            'total_anomalies': len(vessel_anomalies)
                        })
                
                if vessels_with_multiple_types:
                    # Sort by total anomalies (descending)
                    vessels_with_multiple_types.sort(key=lambda x: x['total_anomalies'], reverse=True)
                    
                    report_lines.append("<h2>Vessels with Multiple Anomaly Types</h2>")
                    report_lines.append("<p>The following vessels have been detected with multiple different types of anomalies:</p>")
                    report_lines.append("<table>")
                    report_lines.append("<tr><th>Vessel Name</th><th>MMSI</th><th>Vessel Type</th><th>Anomaly Breakdown</th></tr>")
                    
                    for vessel in vessels_with_multiple_types:
                        ...
                        report_lines.append(f"<td>{vessel['anomaly_breakdown']}</td>")
                        report_lines.append(f"</tr>")
    ...
    def export_vessel_statistics(self, output_path=None):
        ...
        try:
            ...
            if df.empty:
                ...
                timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
                if output_path is None:
                    output_path = os.path.join(str(self.get_output_directory()), f"Vessel_Statistics_{timestamp}.xlsx")
    ...
    def generate_anomaly_timeline(self, output_path=None):
        ...
        try:
            ...
            if output_path is None:
                timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
                output_path = os.path.join(str(self.get_output_directory()), f"Anomaly_Timeline_{timestamp}.html")
    ...
    def correlation_analysis(self, vessel_types, anomaly_types, output_path=None):
        ...
        try:
            ...
            if df.empty and vessel_types_match_run_info:
                ...
                timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
                if output_path is None:
                    output_path = os.path.join(str(self.get_output_directory()), f"Correlation_Analysis_NoData_{timestamp}.html")
                ...
                if not anomaly_df.empty:
                    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
                    if output_path is None:
                        output_path = os.path.join(str(self.get_output_directory()), f"Correlation_Analysis_{timestamp}.html")
    ...
    def temporal_pattern_analysis(self, output_path=None):
        ...
        try:
            ...
            if output_path is None:
                timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
                output_path = os.path.join(str(self.get_output_directory()), f"Temporal_Patterns_{timestamp}.html")
    ...
    def vessel_behavior_clustering(self, vessel_types=None, output_path=None, n_clusters=5):
        ...
        try:
            ...
            if df.empty:
                ...
                timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
                if output_path is None:
                    output_path = os.path.join(str(self.get_output_directory()), f"Vessel_Clustering_NoData_{timestamp}.html")
            ...
            if output_path is None:
                timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
                output_path = os.path.join(str(self.get_output_directory()), f"Vessel_Clustering_{timestamp}.html")
            ...
            # Automatically open HTML report in browser (only if HTML, not CSV)
    ...
    def anomaly_frequency_analysis(self, output_path=None):
        ...
        try:
            ...
            if 'AnomalyType' in anomaly_df.columns:
                ...
                if output_path is None:
                    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
                    output_path = os.path.join(str(self.get_output_directory()), f"Anomaly_Frequency_{timestamp}.html")
    ...
    def create_custom_chart(self, chart_type, x_column=None, y_column=None, color_column=None, 
        ...
        try:
            ...
            if output_path is None:
                timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
                output_path = os.path.join(str(self.get_output_directory()), f"Custom_Chart_{chart_type}_{timestamp}.html")
    ...
    def create_full_spectrum_map(self, show_pins=True, show_heatmap=True, output_path=None):
        ...
        try:
            ...
            # Automatically open HTML map in browser
            self._open_html_file(output_path)
            return output_path
    ...
    def _create_popup_html(self, row):
        """Create HTML for marker popup with professional formatting."""
        ...
        if 'VesselType' in row and pd.notna(row.get('VesselType')):
    ...
    def _open_html_file(self, file_path):
        ...
        try:
            if file_path and os.path.exists(file_path) and file_path.lower().endswith('.html'):
                # Convert to absolute path
                abs_path = os.path.abspath(file_path)
                # Use file:// URL for local files
                file_url = f"file:///{abs_path.replace(os.sep, '/')}"
                webbrowser.open(file_url)
                logger.info(f"Opened HTML report in browser: {file_path}")
                return True
        except Exception as e:
            logger.warning(f"Could not open HTML file automatically: {e}")
            return False
    
    def _create_position_popup_html(self, row, position_num=None):
        """Create HTML popup for position markers with vessel information."""
        html = '<div style="font-family: Arial; font-size: 12px;">'
        html += f'<b>Position {position_num if position_num else ""}</b><br>'
        
        if 'MMSI' in row and pd.notna(row.get('MMSI')):
            html += f'<b>MMSI:</b> {row["MMSI"]}<br>'
        ...
        if 'VesselType' in row and pd.notna(row.get('VesselType')):
            ...
            try:
                type_name = get_vessel_type_name(int(vessel_type))
                html += f'<b>Type:</b> {vessel_type} ({type_name})<br>'
            except:
                html += f'<b>Type:</b> {vessel_type}<br>'
        ...
        if 'SOG' in row and pd.notna(row.get('SOG')):
            html += f'<b>Speed:</b> {row["SOG"]:.2f} knots<br>'
        
        if 'COG' in row and pd.notna(row.get('COG')):
            html += f'<b>Course:</b> {row["COG"]:.1f}°<br>'
        
        if 'Heading' in row and pd.notna(row.get('Heading')):
            html += f'<b>Heading:</b> {row["Heading"]:.1f}°<br>'
        
        html += '</div>'
        return html
    
    def _add_legend(self, m, anomalies_df):
    ...
    def create_vessel_map(self, mmsi, map_types=None, output_path=None, filter_by_anomaly_type=None):
        """
        Create maps focused on specific vessels by MMSI.
        
        Args:
            mmsi: Vessel MMSI
            map_types: List of overlay types to include ('path', 'anomaly', 'heatmap'). 
                       If None or empty, defaults to ['path'].
            output_path: Path to save the map
            filter_by_anomaly_type: Optional list of anomaly types to filter
        """
        try:
            if not FOLIUM_AVAILABLE:
                logger.error("folium not available for map creation")
            ...
            # Handle map_types - support both single string and list
            if map_types is None:
                map_types = ['path']
            elif isinstance(map_types, str):
                map_types = [map_types]
            elif not isinstance(map_types, list):
                map_types = [map_types]
            
            # Remove duplicates and validate
            map_types = list(set([mt.lower() for mt in map_types if mt.lower() in ['path', 'anomaly', 'heatmap']]))
            if not map_types:
                map_types = ['path']
            
            logger.info(f"Creating vessel map for {mmsi} with overlays: {map_types}")
            ...
            # Filter anomalies by type if specified
            if not vessel_anomalies.empty and filter_by_anomaly_type and 'AnomalyType' in vessel_anomalies.columns:
                vessel_anomalies = vessel_anomalies[
                    vessel_anomalies['AnomalyType'].isin(filter_by_anomaly_type)
                ].copy()
                logger.info(f"Filtered anomalies to types: {filter_by_anomaly_type} ({len(vessel_anomalies)} remaining)")
            
            if output_path is None:
                timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
                map_dir = self.get_map_output_directory()
                overlay_str = "_".join(map_types)
                output_path = os.path.join(map_dir, f"Vessel_{mmsi}_{overlay_str}_{timestamp}.html")
            ...
            title_html = f'''
                <div style="position: fixed; 
                    ...
                    <span style="font-size: 12px; font-weight: normal;">
                        ...
                        {len(vessel_data)} positions tracked{date_range_str}{filter_info}<br>
                        Overlays: {', '.join([mt.title() for mt in map_types])}
                    </span>
                </div>
            '''
            m.get_root().html.add_child(folium.Element(title_html))
            
            # Create FeatureGroups for each overlay type so they can be toggled
            path_group = folium.FeatureGroup(name='Path Track', overlay=True, control=True, show='path' in map_types)
            anomaly_group = folium.FeatureGroup(name='Anomaly Markers', overlay=True, control=True, show='anomaly' in map_types)
            heatmap_group = folium.FeatureGroup(name='Heatmap', overlay=True, control=True, show='heatmap' in map_types)
            ...
            if 'path' in map_types:
                ...
                if path_coords:
                    folium.PolyLine(path_coords, color='blue', weight=3, opacity=0.7, 
                                   popup='Vessel Track', tooltip=f'Track with {len(path_coords)} points').add_to(path_group)
                    
                    # Add diamond markers for each position along the path track
                    # These markers are clickable and show position details in popups
                    # Create a list of valid rows (with coordinates) for matching
                    valid_rows = vessel_data[vessel_data['LAT'].notna() & vessel_data['LON'].notna()].copy()
                    if 'BaseDateTime' in valid_rows.columns:
                        valid_rows = valid_rows.sort_values('BaseDateTime')
                    
                    for idx, coord in enumerate(path_coords):
                        # Get the corresponding row data for popup
                        row_data = None
                        if idx < len(valid_rows):
                            row_data = valid_rows.iloc[idx]
                        else:
                            # Fallback: get row by matching coordinates
                            matching_rows = valid_rows[
                                (valid_rows['LAT'].round(4) == round(coord[0], 4)) & 
                                (valid_rows['LON'].round(4) == round(coord[1], 4))
                            ]
                            row_data = matching_rows.iloc[0] if not matching_rows.empty else None
                        
                        # Create popup HTML
                        popup_html = self._create_position_popup_html(row_data, idx + 1) if row_data is not None else f'Position {idx + 1}'
                        
                        # Create a diamond-shaped marker using DivIcon
                        diamond_html = '''
                        <div style="
                            width: 6px;
                            height: 6px;
                        ...
                        folium.Marker(
                            ...
                            tooltip=f'Position {idx + 1}'
                        ).add_to(path_group)
                    
                    folium.Marker(path_coords[0], popup='Start', tooltip='Track Start',
                                 icon=folium.Icon(color='green', icon='play', prefix='fa')).add_to(path_group)
                    folium.Marker(path_coords[-1], popup='End', tooltip='Track End',
                                 icon=folium.Icon(color='red', icon='stop', prefix='fa')).add_to(path_group)
                    logger.info(f"Path overlay: Added {len(path_coords)} points with diamond markers for vessel {mmsi}")
                else:
                    logger.warning(f"No valid coordinates found for path overlay")
            
            # Add anomaly overlay with interactive filtering by type
            if 'anomaly' in map_types and not vessel_anomalies.empty:
                # Group anomalies by type for interactive filtering
                if 'AnomalyType' in vessel_anomalies.columns:
                    anomaly_types = vessel_anomalies['AnomalyType'].unique()
                    total_markers = 0
                    
                    for anomaly_type in sorted(anomaly_types):
                        type_anomalies = vessel_anomalies[vessel_anomalies['AnomalyType'] == anomaly_type]
                        type_count = len(type_anomalies)
                        
                        # Create a FeatureGroup for each anomaly type
                        type_group = folium.FeatureGroup(
                            name=f'Anomaly: {anomaly_type} ({type_count})',
                            overlay=True,
                            control=True,
                        ...
                        marker_cluster = MarkerCluster().add_to(type_group)
                        ...
                        for _, row in type_anomalies.iterrows():
                            if pd.notna(row.get('LAT')) and pd.notna(row.get('LON')):
                                ...
                                folium.Marker(
                                    [row['LAT'], row['LON']],
                                    popup=folium.Popup(popup_html, max_width=300),
                                    tooltip=f"{anomaly_type} - MMSI: {mmsi}",
                                    icon=folium.Icon(color=color, icon=icon, prefix='fa')
                                ).add_to(marker_cluster)
                                total_markers += 1
                        
                        logger.info(f"Anomaly overlay: Added {type_count} {anomaly_type} markers")
                    
                    logger.info(f"Anomaly overlay: Added {total_markers} total markers for vessel {mmsi}")
                else:
                    # Fallback if no AnomalyType column
                    ...
                    for _, row in vessel_anomalies.iterrows():
                        if pd.notna(row.get('LAT')) and pd.notna(row.get('LON')):
                            ...
                            marker_count += 1
                    logger.info(f"Anomaly overlay: Added {marker_count} markers for vessel {mmsi}")
            elif 'anomaly' in map_types:
                logger.warning(f"No anomalies found for vessel {mmsi}")
            
            # Add heatmap overlay - ONLY use selected anomalies (never use path data)
            if 'heatmap' in map_types:
                heatmap_group.add_to(m)
                # Only use selected anomalies for heatmap (already filtered by filter_by_anomaly_type if provided)
                if not vessel_anomalies.empty:
                    heat_data = [[row['LAT'], row['LON'], 1] for _, row in vessel_anomalies.iterrows()
                                if pd.notna(row.get('LAT')) and pd.notna(row.get('LON'))]
                    if heat_data:
                        HeatMap(heat_data, radius=15, blur=25, max_zoom=13).add_to(heatmap_group)
                        logger.info(f"Heatmap overlay: Added {len(heat_data)} heat points from selected anomalies for vessel {mmsi}")
                    else:
                        logger.warning(f"No valid anomaly coordinates found for heatmap overlay for vessel {mmsi}")
                else:
                    logger.info(f"No selected anomalies found for vessel {mmsi} - skipping heatmap overlay")
            ...
            # Add legend if anomalies are present
            if not vessel_anomalies.empty and 'anomaly' in map_types:
                self._add_legend(m, vessel_anomalies)
    ...
    def create_filtered_map(self, map_types=None, vessel_types=None, anomaly_types=None, vessel_mmsi=None, output_path=None, use_full_datasets=False, selected_days=None):
        """
        ...
        Args:
            ...
            selected_days: List of date objects to filter by. If None, includes all days.
        """
        try:
            if not FOLIUM_AVAILABLE:
                logger.error("folium not available for map creation")
            ...
            logger.info(f"Creating filtered map with overlays {map_types}: vessel_types={vessel_types}, anomaly_types={anomaly_types}, vessel_mmsi={vessel_mmsi}, use_full_datasets={use_full_datasets}")
            ...
            if selected_days is not None:
                # Convert selected_days to date objects if they're strings
                if selected_days and len(selected_days) > 0:
                    if isinstance(selected_days[0], str):
                        selected_dates = [pd.to_datetime(d).date() for d in selected_days]
                    else:
                    ...
                    # Filter anomaly_df by date
                    if not anomaly_df.empty:
                        if 'BaseDateTime' in anomaly_df.columns:
                            anomaly_df['Date'] = pd.to_datetime(anomaly_df['BaseDateTime']).dt.date
                            anomaly_df = anomaly_df[anomaly_df['Date'].isin(selected_dates)].copy()
                        elif 'ReportDate' in anomaly_df.columns:
                            anomaly_df['Date'] = pd.to_datetime(anomaly_df['ReportDate']).dt.date
                            anomaly_df = anomaly_df[anomaly_df['Date'].isin(selected_dates)].copy()
            ...
            group_by_day = selected_days is not None and len(selected_days) > 1
            
            # Get unique days from data if grouping by day
            if group_by_day:
                if 'Date' in df.columns:
                    unique_days = sorted(df['Date'].unique().tolist())
                elif 'BaseDateTime' in df.columns:
                    unique_days = sorted(pd.to_datetime(df['BaseDateTime']).dt.date.unique().tolist())
                elif 'ReportDate' in df.columns:
                    unique_days = sorted(pd.to_datetime(df['ReportDate']).dt.date.unique().tolist())
                else:
                    group_by_day = False
                    unique_days = []
            else:
            ...
            if output_path is None:
                ...
                map_dir = self.get_map_output_directory()
                overlay_str = "_".join(map_types)
                filter_str = f"filtered_{overlay_str}_{timestamp}"
                if vessel_mmsi:
                    filter_str = f"vessel_{vessel_mmsi}_{overlay_str}_{timestamp}"
                output_path = os.path.join(map_dir, f"{filter_str}.html")
            
            # Calculate center and create map
            center_lat, center_lon = self._calculate_center(df)
            m = folium.Map(
                location=[center_lat, center_lon],
                zoom_start=6,
                tiles='OpenStreetMap',
            ...
            filter_info_parts = []
            if anomaly_types:
                filter_info_parts.append(f"Anomaly Types: {', '.join(anomaly_types)}")
            if vessel_types:
                filter_info_parts.append(f"Vessel Types: {', '.join(map(str, vessel_types))}")
            if vessel_mmsi:
                filter_info_parts.append(f"MMSI: {vessel_mmsi}")
            
            filter_info = ""
            if filter_info_parts:
                filter_info = f"<br><span style='font-size: 11px; color: #666;'>Filters: {' | '.join(filter_info_parts)}</span>"
                original_count = len(anomaly_df) if not anomaly_df.empty else 0
                if original_count > 0:
                    filter_info += f"<br><span style='font-size: 11px; color: #666;'>({len(anomaly_df)}/{original_count} anomalies shown)</span>"
            ...
            title_html = f'''
                <div style="position: fixed; 
                    ...
                    AIS Filtered Map<br>
                    <span style="font-size: 12px; font-weight: normal;">
                        {len(df)} vessels, {len(anomaly_df)} anomalies{filter_info}<br>
                        Overlays: {', '.join([mt.title() for mt in map_types])}
                    </span>
                </div>
            '''
            ...
            # Create FeatureGroups - either by day or global
            if group_by_day:
                # Create day-based FeatureGroups
                day_groups = {}
                for day in unique_days:
                    day_str = day.strftime('%Y-%m-%d')
                    day_group = folium.FeatureGroup(name=f'Day: {day_str}', overlay=False, control=True, show=True)
                    day_group.add_to(m)
                    day_groups[day] = day_group
                    
                    # Create overlay groups for this day
                    day_path_group = folium.FeatureGroup(name=f'{day_str} - Path Tracks', overlay=True, control=True, show='path' in map_types)
                    day_anomaly_group = folium.FeatureGroup(name=f'{day_str} - Anomaly Markers', overlay=True, control=True, show='anomaly' in map_types)
                    day_heatmap_group = folium.FeatureGroup(name=f'{day_str} - Heatmap', overlay=True, control=True, show='heatmap' in map_types)
                    
                    day_path_group.add_to(day_group)
                    day_anomaly_group.add_to(day_group)
                    day_heatmap_group.add_to(day_group)
                    
                    day_groups[day] = {
                        'parent': day_group,
                        'path': day_path_group,
                        'anomaly': day_anomaly_group,
                        'heatmap': day_heatmap_group
                    }
            else:
                # Create global FeatureGroups (no day grouping)
                path_group = folium.FeatureGroup(name='Path Tracks', overlay=True, control=True, show='path' in map_types)
                anomaly_group = folium.FeatureGroup(name='Anomaly Markers', overlay=True, control=True, show='anomaly' in map_types)
                heatmap_group = folium.FeatureGroup(name='Heatmap', overlay=True, control=True, show='heatmap' in map_types)
            ...
            if 'path' in map_types:
                if group_by_day:
                    # Add paths grouped by day
                    for day in unique_days:
                        day_data = df[df['Date'] == day] if 'Date' in df.columns else df
                        if 'Date' not in day_data.columns and 'BaseDateTime' in day_data.columns:
                            day_data = day_data[pd.to_datetime(day_data['BaseDateTime']).dt.date == day]
                        elif 'Date' not in day_data.columns and 'ReportDate' in day_data.columns:
                            day_data = day_data[pd.to_datetime(day_data['ReportDate']).dt.date == day]
                        
                        day_path_group = day_groups[day]['path']
                        # Group by MMSI and draw paths
                        for mmsi in day_data['MMSI'].unique():
                            vessel_data = day_data[day_data['MMSI'] == mmsi]
                            ...
                            if path_coords:
                                folium.PolyLine(path_coords, color='blue', weight=2, opacity=0.7, 
                                               popup=f"Vessel {mmsi} - {day.strftime('%Y-%m-%d')}", 
                                               tooltip=f"Vessel {mmsi} - {day.strftime('%Y-%m-%d')}").add_to(day_path_group)
                        logger.info(f"Path overlay: Added paths for {len(day_data['MMSI'].unique())} vessels on {day.strftime('%Y-%m-%d')}")
                else:
                    # Add paths without day grouping
                    path_group.add_to(m)
                    # Group by MMSI and draw paths
                    for mmsi in df['MMSI'].unique():
                        vessel_data = df[df['MMSI'] == mmsi]
                        ...
                        if path_coords:
                            folium.PolyLine(path_coords, color='blue', weight=2, opacity=0.7, 
                                           popup=f"Vessel {mmsi}", tooltip=f"Vessel {mmsi}").add_to(path_group)
                    logger.info(f"Path overlay: Added paths for {len(df['MMSI'].unique())} vessels")
            
            # Add anomaly overlay
            if 'anomaly' in map_types and not anomaly_df.empty:
                if group_by_day:
                    # Add anomalies grouped by day
                    for day in unique_days:
                        day_anomaly_group = day_groups[day]['anomaly']
                        day_anomaly_data = anomaly_df[anomaly_df['Date'] == day] if 'Date' in anomaly_df.columns else pd.DataFrame()
                        if 'Date' not in day_anomaly_data.columns and 'BaseDateTime' in anomaly_df.columns:
                            day_anomaly_data = anomaly_df[pd.to_datetime(anomaly_df['BaseDateTime']).dt.date == day]
                        elif 'Date' not in day_anomaly_data.columns and 'ReportDate' in anomaly_df.columns:
                            day_anomaly_data = anomaly_df[pd.to_datetime(anomaly_df['ReportDate']).dt.date == day]
                        
                        if not day_anomaly_data.empty and 'AnomalyType' in day_anomaly_data.columns:
                            anomaly_types_list = day_anomaly_data['AnomalyType'].unique()
                            day_total_markers = 0
                            
                            for anomaly_type in sorted(anomaly_types_list):
                                type_anomalies = day_anomaly_data[day_anomaly_data['AnomalyType'] == anomaly_type]
                                type_count = len(type_anomalies)
                                
                                # Create a FeatureGroup for each anomaly type within this day
                                type_group = folium.FeatureGroup(
                                    name=f'{day.strftime("%Y-%m-%d")} - {anomaly_type} ({type_count})',
                                    overlay=True,
                                    control=True,
                                    show=True
                                )
                                type_group.add_to(day_anomaly_group)
                                ...
                                for _, row in type_anomalies.iterrows():
                                    if pd.notna(row.get('LAT')) and pd.notna(row.get('LON')):
                                        ...
                                        folium.Marker(
                                            [row['LAT'], row['LON']],
                                            popup=folium.Popup(popup_html, max_width=300),
                                            tooltip=f"{anomaly_type} - MMSI: {row.get('MMSI', 'N/A')} - {day.strftime('%Y-%m-%d')}",
                                            icon=folium.Icon(color=color, icon=icon, prefix='fa')
                                        ).add_to(marker_cluster)
                                        day_total_markers += 1
                                
                                logger.info(f"Anomaly overlay: Added {type_count} {anomaly_type} markers for {day.strftime('%Y-%m-%d')}")
                            
                            logger.info(f"Anomaly overlay: Added {day_total_markers} total markers for {day.strftime('%Y-%m-%d')}")
                else:
                    # Add anomalies without day grouping
                    # Group anomalies by type for interactive filtering
                    if 'AnomalyType' in anomaly_df.columns:
                        anomaly_types_list = anomaly_df['AnomalyType'].unique()
                        total_markers = 0
                        
                        for anomaly_type in sorted(anomaly_types_list):
                            type_anomalies = anomaly_df[anomaly_df['AnomalyType'] == anomaly_type]
                            type_count = len(type_anomalies)
                        ...
                        logger.info(f"Anomaly overlay: Added {total_markers} total markers")
                    else:
                        # Fallback if no AnomalyType column
                        anomaly_group.add_to(m)
                        marker_cluster = MarkerCluster().add_to(anomaly_group)
                        ...
                        for _, row in anomaly_df.iterrows():
                            if pd.notna(row.get('LAT')) and pd.notna(row.get('LON')):
                                ...
                                marker_count += 1
                        logger.info(f"Anomaly overlay: Added {marker_count} markers")
            elif 'anomaly' in map_types:
                logger.warning("No anomalies found for filtered anomaly overlay")
            
            # Add heatmap overlay - ONLY use selected anomalies (never fall back to path data)
            if 'heatmap' in map_types:
                if group_by_day:
                    # Add heatmaps grouped by day - only from selected anomalies
                    for day in unique_days:
                        day_heatmap_group = day_groups[day]['heatmap']
                        
                        # Get anomaly data for this day (already filtered by selected anomaly types)
                        day_anomaly_data = anomaly_df[anomaly_df['Date'] == day] if 'Date' in anomaly_df.columns and not anomaly_df.empty else pd.DataFrame()
                        if 'Date' not in day_anomaly_data.columns and not anomaly_df.empty:
                            if 'BaseDateTime' in anomaly_df.columns:
                                day_anomaly_data = anomaly_df[pd.to_datetime(anomaly_df['BaseDateTime']).dt.date == day]
                            elif 'ReportDate' in anomaly_df.columns:
                                day_anomaly_data = anomaly_df[pd.to_datetime(anomaly_df['ReportDate']).dt.date == day]
                        
                        # Only create heatmap from selected anomalies
                        if not day_anomaly_data.empty:
                            heat_data = [[row['LAT'], row['LON'], 1] for _, row in day_anomaly_data.iterrows()
                                        if pd.notna(row.get('LAT')) and pd.notna(row.get('LON'))]
                            
                            if heat_data:
                                HeatMap(heat_data, radius=15, blur=25, max_zoom=13).add_to(day_heatmap_group)
                                logger.info(f"Heatmap overlay: Added {len(heat_data)} heat points from selected anomalies for {day.strftime('%Y-%m-%d')}")
                            else:
                                logger.warning(f"No valid anomaly coordinates found for heatmap overlay for {day.strftime('%Y-%m-%d')}")
                        else:
                            logger.info(f"No selected anomalies found for {day.strftime('%Y-%m-%d')} - skipping heatmap for this day")
                else:
                    # Add heatmap without day grouping - only from selected anomalies
                    heatmap_group.add_to(m)
                    if not anomaly_df.empty:
                        # Only use selected anomalies for heatmap
                        heat_data = [[row['LAT'], row['LON'], 1] for _, row in anomaly_df.iterrows()
                                    if pd.notna(row.get('LAT')) and pd.notna(row.get('LON'))]
                        
                        if heat_data:
                            HeatMap(heat_data, radius=15, blur=25, max_zoom=13).add_to(heatmap_group)
                            logger.info(f"Heatmap overlay: Added {len(heat_data)} heat points from selected anomalies")
                        else:
                            logger.warning("No valid anomaly coordinates found for heatmap overlay")
                    else:
                        logger.info("No selected anomalies found - skipping heatmap overlay")
            ...
            # Add legend if anomalies are present
            if not anomaly_df.empty and 'anomaly' in map_types:
                self._add_legend(m, anomaly_df)
...
class AdvancedAnalysisGUI:
    ...
    def _create_vessel_map_dialog(self):
        ...
        dialog = tk.Toplevel(self.window)
        dialog.title("Vessel Map Overlays")
        dialog.geometry("400x500")
        
        ttk.Label(dialog, text="Select Overlays (multiple can be selected):", font=("Arial", 10, "bold")).pack(pady=5)
        
        # Use checkboxes for multiple selection
        path_var = tk.BooleanVar(value=True)  # Default to path
        anomaly_var = tk.BooleanVar(value=False)
        heatmap_var = tk.BooleanVar(value=False)
        
        ttk.Checkbutton(dialog, text="Path Track", variable=path_var).pack(anchor=tk.W, padx=20, pady=2)
        ttk.Checkbutton(dialog, text="Anomaly Markers", variable=anomaly_var).pack(anchor=tk.W, padx=20, pady=2)
        ttk.Checkbutton(dialog, text="Heatmap", variable=heatmap_var).pack(anchor=tk.W, padx=20, pady=2)
        
        # Anomaly type filter (optional)
        try:
            anomaly_df = self.analysis.load_anomaly_data()
            available_anomaly_types_data = sorted(anomaly_df['AnomalyType'].unique().tolist()) if not anomaly_df.empty and 'AnomalyType' in anomaly_df.columns else []
            available_anomaly_types_gui = [map_anomaly_type_data_to_gui(at) for at in available_anomaly_types_data]
            
            if available_anomaly_types_gui:
                ttk.Label(dialog, text="Filter by Anomaly Type (optional):", font=("Arial", 10, "bold")).pack(pady=(10, 5))
                anomaly_frame = ttk.Frame(dialog)
                anomaly_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=5)
                
                anomaly_listbox = tk.Listbox(anomaly_frame, selectmode=tk.MULTIPLE, height=6)
                anomaly_listbox.pack(fill=tk.BOTH, expand=True)
                for at in available_anomaly_types_gui:
                    anomaly_listbox.insert(tk.END, at)
                
                scrollbar = ttk.Scrollbar(anomaly_frame, orient=tk.VERTICAL, command=anomaly_listbox.yview)
                scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
                anomaly_listbox.config(yscrollcommand=scrollbar.set)
        except Exception as e:
            logger.warning(f"Could not load anomaly types for filtering: {e}")
            anomaly_listbox = None
        
        def create_map():
            # Collect selected overlay types
            selected_map_types = []
            if path_var.get():
            ...
            if not selected_map_types:
                messagebox.showerror("Error", "Please select at least one overlay type")
                return
            
            # Get selected anomaly types BEFORE destroying the dialog
            # This is critical - widgets are destroyed when dialog.destroy() is called
            filter_by_anomaly_type = None
            try:
                if anomaly_listbox and hasattr(anomaly_listbox, 'curselection'):
                    selected_indices = anomaly_listbox.curselection()
                    if selected_indices:
                        selected_gui_types = [anomaly_listbox.get(i) for i in selected_indices]
                        filter_by_anomaly_type = [map_anomaly_type_gui_to_data(gui_name) for gui_name in selected_gui_types]
            except Exception as e:
                logger.warning(f"Error reading anomaly listbox selection: {e}")
                filter_by_anomaly_type = None
            
            dialog.destroy()
            progress = ProgressDialog(self.window, "Creating Map", f"Creating map with {len(selected_map_types)} overlay(s)...")
            try:
                
                self.status_var.set(f"Creating map for vessel {mmsi} with overlays: {', '.join(selected_map_types)}...")
                result = self.analysis.create_vessel_map(mmsi, map_types=selected_map_types, filter_by_anomaly_type=filter_by_anomaly_type)
                progress.close()
                if result:
                    self.status_var.set(f"Map created: {result}")
                    messagebox.showinfo("Success", f"Map created:\n{result}")
    ...
    def _create_filtered_map_dialog(self):
        ...
        try:
            ...
            if not df.empty:
                if 'BaseDateTime' in df.columns:
                    try:
                        df_dates = pd.to_datetime(df['BaseDateTime']).dt.date
                        available_dates = sorted(df_dates.unique().tolist())
                    except:
                        pass
                elif 'ReportDate' in df.columns:
                    try:
                        df_dates = pd.to_datetime(df['ReportDate']).dt.date
                        available_dates = sorted(df_dates.unique().tolist())
                    except:
                        pass
            ...
            if not anomaly_df.empty:
                if 'BaseDateTime' in anomaly_df.columns:
                    try:
                        anomaly_dates = pd.to_datetime(anomaly_df['BaseDateTime']).dt.date
                        available_dates.extend(anomaly_dates.unique().tolist())
                    except:
                        pass
                elif 'ReportDate' in anomaly_df.columns:
                    try:
                        anomaly_dates = pd.to_datetime(anomaly_df['ReportDate']).dt.date
                        available_dates.extend(anomaly_dates.unique().tolist())
                    except:
                        pass
            ...
            # Create button frame at the top (will be populated after variables are defined)
            button_frame = ttk.Frame(dialog, padding=10)
            button_frame.pack(fill=tk.X, side=tk.TOP)
            
            # Day selection (if dates are available)
            day_vars = {}
            day_checkboxes = []
            if available_dates:
                ttk.Label(dialog, text="Select Days (multiple can be selected):", font=("Arial", 10, "bold")).pack(pady=5)
                day_frame = ttk.Frame(dialog)
                day_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=5)
                
                # Add "All Days" checkbox
                all_days_var = tk.BooleanVar(value=True)  # Default to all days
                ttk.Checkbutton(day_frame, text="All Days", variable=all_days_var).pack(anchor=tk.W, pady=2)
                
                # Add individual day checkboxes
                for date in available_dates:
                    var = tk.BooleanVar(value=True)  # Default to selected
                    day_vars[date] = var
                    date_str = date.strftime('%Y-%m-%d')
                    cb = ttk.Checkbutton(day_frame, text=date_str, variable=var)
                    cb.pack(anchor=tk.W, padx=20, pady=1)
                    day_checkboxes.append((date, var))
                
                # Function to sync "All Days" with individual days
                def sync_all_days(*args):
                    if all_days_var.get():
                        for date, var in day_checkboxes:
                            var.set(True)
                
                # Function to sync individual days with "All Days"
                def sync_individual_days(*args):
                    all_selected = all(var.get() for _, var in day_checkboxes)
                    # Use after_idle to avoid recursion issues
                    dialog.after_idle(lambda: all_days_var.set(all_selected))
                
                all_days_var.trace('w', sync_all_days)
                for date, var in day_checkboxes:
                    var.trace('w', sync_individual_days)
            
            # Map overlay selection (multiple can be selected)
            ttk.Label(dialog, text="Select Overlays (multiple can be selected):", font=("Arial", 10, "bold")).pack(pady=5)
            ...
            ttk.Checkbutton(dialog, text="Path Tracks", variable=path_var).pack(anchor=tk.W, padx=20, pady=2)
            ttk.Checkbutton(dialog, text="Anomaly Markers", variable=anomaly_var).pack(anchor=tk.W, padx=20, pady=2)
            ttk.Checkbutton(dialog, text="Heatmap", variable=heatmap_var).pack(anchor=tk.W, padx=20, pady=2)
            ...
            # Define create_map function after all variables are defined
            def create_map():
                vessel_mmsi = None
                ...
                if available_dates:
                    if all_days_var.get():
                        selected_days = None  # None means all days
                    else:
                        selected_days = [date for date, var in day_checkboxes if var.get()]
                        if not selected_days:
                            messagebox.showerror("Error", "Please select at least one day or 'All Days'")
                            return
                
                dialog.destroy()
                progress = ProgressDialog(self.window, "Creating Map", f"Creating filtered map with {len(selected_map_types)} overlay(s)...")
                try:
                    self.status_var.set(f"Creating filtered map with overlays: {', '.join(selected_map_types)}...")
                    result = self.analysis.create_filtered_map(
            ...
            # Add buttons to the button frame (already created at top)
            ttk.Button(button_frame, text="Create Map", command=create_map).pack(side=tk.LEFT, padx=5)
            ttk.Button(button_frame, text="Cancel", command=dialog.destroy).pack(side=tk.RIGHT, padx=5)
    ...
    def _display_prediction_results(self, result: dict):
        ...
        try:
            ...
            # Automatically open HTML map in browser
            if self.analysis:
                self.analysis._open_html_file(str(map_path))
            else:
                # Fallback: use webbrowser directly
                try:
                    import webbrowser
                    abs_path = os.path.abspath(str(map_path))
                    file_url = f"file:///{abs_path.replace(os.sep, '/')}"
                    webbrowser.open(file_url)
                    logger.info(f"Opened prediction map in browser: {map_path}")
                except Exception as e:
                    logger.warning(f"Could not open prediction map automatically: {e}")
